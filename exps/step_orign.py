...
def create_shared_dataset(splitedData, percentage=0.2, seed=None, stratified=True):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    for ds_name, (dataloaders, _,_ , _) in splitedData.items():
        ...
        # 深拷贝并添加到共享数据集
        shared_dataset.extend([ensure_edge_attributes(copy.deepcopy(g)) for g in selected_graphs])
...
def create_pretrain_dataset(splitedData, percentage=0.6, seed=None, stratified=True):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    for ds_name, (dataloaders, _, _, _) in splitedData.items():
        ...
        for g in selected_graphs:
            g = ensure_edge_attributes(copy.deepcopy(g))
            # 👇补全 num_nodes 属性
            if not hasattr(g, 'num_nodes') or g.num_nodes is None:
                if hasattr(g, 'x') and g.x is not None:
                    g.num_nodes = g.x.size(0)
                else:
                    g.num_nodes = 0  # fallback：如果没有 x，设置为 0，视情况处理
            pretrain_dataset.append(g)
...
def get_padded_dataloader(original_dataloader, global_node_dim, global_edge_dim, global_label_dim, batch_size, shuffle):
    """
    根据原始 DataLoader 得到经过填充处理的 DataLoader
    """
...
def process_client_dataloaders(dataloaders, global_node_dim, global_edge_dim, global_label_dim, batch_size):
    """
    对客户端的 train, val, test 数据集分别进行填充处理，返回更新后的 dataloaders。
    """
...
def setup_devices_SSP(splitedData, args):
    ...
    # 创建共享数据集
    shared_dataset = create_shared_dataset(splitedData, percentage=0.2, seed=args.seed)
    ...
    for idx, ds in enumerate(splitedData.keys()):
        ...
        # 将填充后的共享数据集加入客户端 dataloaders
        dataloaders['shared'] = shared_loader
        
        # 分别对 train, val, test 数据集进行填充
        ...
        if args.alg == "fedSSP":
            former = SSP(global_label_dim, args.nlayer, node_feature_dim, 0, node_feature_dim[0], args.head, args.hidden)
            head = former.fc
    ...
    # 初始化服务器模型（使用全局维度）
    server_node_feature = [global_node_dim]  # 与客户端保持相同的结构格式
    ...
    server = Server(server_model, args.device, shared_loader, pretrain_loader, args)  # 将共享数据集传入 server
